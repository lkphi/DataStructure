...
inline MGraph<T>::MGraph(int vertexCount, bool isDirected)
{
	try
	{
		...
		if (!mGraphMatrix) throw FailedApplyForSpace();
		for (int i = 0; i < vertexCount; i++)
		{
			mGraphMatrix[i] = new int[vertexCount] {0};
			if (!mGraphMatrix[i]) throw FailedApplyForSpace();
		}
		//mDataArray = new int[vertexCount];
		//if (!mDataArray) throw;
		mVisitedArray = new bool[vertexCount];
		if (!mVisitedArray) throw FailedApplyForSpace();
...
#pragma region apply for memory
		...
		if (!mShortestPathLastIndex) throw FailedApplyForSpace();
		if (!mShortestDist)
			mShortestDist = new int[mVertexCount];
		if (!mShortestDist) throw FailedApplyForSpace();
#pragma endregion
...
#pragma region apply for memeory
		if (!mShortestDistMatrix)
		{
			...
			if (!mShortestDistMatrix) throw FailedApplyForSpace();
			for (int i = 0; i < mVertexCount; i++)
			{
				mShortestDistMatrix[i] = new int[mVertexCount] {0};
				if (!mShortestDistMatrix[i]) throw FailedApplyForSpace();
			}
		}
		if (!mShortestPathLastIndexMatrix)
		{
			...
			for (int i = 0; i < mVertexCount; i++)
			{
				...
				if (!mShortestPathLastIndexMatrix[i]) throw FailedApplyForSpace();
			}
		}
#pragma endregion
...
inline int MGraph<T>::FindMinPoint2OtherPoint()
{
	try
	{
		...
		if (!maxWeightArray)throw FailedApplyForSpace();
		for (int i = 0; i < mVertexCount; ++i)
			for (int j = 0; j < mVertexCount; ++j)
			{
...
inline bool MGraph<T>::Prim(int rootIndex)//O(V^2) 适用于稠密图
{
	try
	{
		...
		if (!dist) throw FailedApplyForSpace();
		if (!mMSTParent)
			mMSTParent = new int[mVertexCount] {-1};//父节点的编号
		if (!mMSTParent) throw FailedApplyForSpace();
		for (int i = 0; i < mVertexCount; i++)
		{
...
//每个顶点都是一棵树，通过边把两颗树并成一颗树，把所有的顶点并成一棵树

template<typename T>
inline void MGraph<T>::Kruskal()//采用堆O(ElogE)，适合顶点和边差不多，边比较少
{
	try
	{
		int** graphMatrix = new int* [mVertexCount];
		if (!graphMatrix) throw FailedApplyForSpace();
		for (int i = 0; i < mVertexCount; i++)
		{
			graphMatrix[i] = new int[mVertexCount];
			if (!graphMatrix[i]) throw FailedApplyForSpace();
			for (int j = 0; j < mVertexCount; j++)
...
inline void MGraph<T>::TopSort()//O(|V|^2)
{
	try
	{
		...
		if (!indegree) throw FailedApplyForSpace();
		queue<int>* vertexQueue = new queue<int>();
		if (!vertexQueue) throw FailedApplyForSpace();
		queue<int>* sortQueue = new queue<int>();
		if (!sortQueue) throw FailedApplyForSpace();
		for (int i = 0; i < mVertexCount; ++i)
		{
			for (int j = 0; j < mVertexCount; ++j)
			{
...
inline void TGraph<T>::MinHeapDijkstra(GraphIndex start)//T = O(| V | log | v | +| E | log | V | ) = O(| E | log | V | ), 对于稀疏图效果好
{
	try
	{
		...
		delete[] array;
		array = nullptr;
		if (!minHeap) throw FailedApplyForSpace();
		for (int i = 0; i < mVertexCount; i++)
		{
			do {
				minWeightIndex = (minHeap->RemoveRoot()).v2;
...
inline void TGraph<T>::Kruskal()
{
	try
	{
		...
		if (!array) throw FailedApplyForSpace();
...
inline TravelPlanGraphic::TravelPlanGraphic(int vertexCount)
{
	try
	{
		...
		if (!mGraphicTable) throw FailedApplyForSpace();
		mVertexCount = vertexCount;
		mEdgeCount = 0;
	}
	catch (const std::exception& e)
...
#pragma region apply for memory
		...
		if (!shortestPathLastIndex) throw FailedApplyForSpace();
		TPFactor* shortestDist = new TPFactor[mVertexCount];
		if (!shortestDist) throw FailedApplyForSpace();
		for (int i = 0; i < mVertexCount; i++)
		{
